...
<?php
if(!isset($_GET['nama'])){?>
<div id="title_page">TAMBAH LOKASI</div>
<?php
}
...
if(isset($_GET['nama'])){
	...
	$qrysimpan = que("INSERT INTO  `db_autrom`.`tb_lokasi` (
				`id_lokasi` ,
				`lokasi_block` ,
				`jumlah_sekat`
				)
				VALUES (
				NULL ,  '$_GET[nama]',  '$_GET[jumlah]'
				);");	
	if($qrysimpan)
...
$(document).ready(function(){
	$("#formtambah").submit(function(event){
		...
		$.ajax({
			url: "page/lokasi_tambah.php?"+data,
			success: function(result,status){				
				...
				{halaman('page/lokasi_tambah.php');}
			}
		});
	});
});
...
<form id="formtambah" action="#" method="get">
    <table id="form">
      <tr>
        ...
        <td>
        	<input type="text" name='nama' required id="input"/>
      ...
      <tr>
        ...
        <td>
        	<input type="text" name='jumlah' required id="input"/>
